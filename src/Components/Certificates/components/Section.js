...
import { buttonStyle } from "./service.js";
    ...
    render() {
        return (
            <section className="buttonCert">
                <h3 className="title-sert light_dark">{this.state.lang==='EN'? this.state.object.en: this.state.lang==='UA'? this.state.object.ua: null}</h3>
                ...
                <ul className={"checkbox_block_"+this.state.object.type}>
                    {this.state.certificates.map((cert, ind) => <Checkbox key={ind} certificate={cert} lang={this.state.lang} object={this.state.object} len={this.state.certificates.length}/>)}
                </ul>
            </section>
        );
    }
